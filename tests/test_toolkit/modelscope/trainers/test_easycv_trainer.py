...
def train_func(work_dir, dist=False, log_interval=3, imgs_per_gpu=4):
    ...
    if os.path.exists(os.path.join(easycv_dir, config_file)):
        config_path = os.path.join(easycv_dir, config_file)
    else:
        config_path = os.path.join(os.path.dirname(easycv_dir), config_file)
