...
from easycv.datasets.detection3d.data_sources import Det3dSourceNuScenes
...
def get_available_scenes(nusc):
    ...
    for scene in nusc.scene:
        ...
        while has_more_frames:
            ...
            # path from lyftdataset is absolute path, convert to relative path
            lidar_path = lidar_path.split(args.root_path)[-1]
...
def obtain_sensor2top(nusc,
    ...
    # path from lyftdataset is absolute path, convert to relative path
    data_path = data_path.split(args.root_path)[-1]
    sweep = {
        'data_path': data_path,
        'type': sensor_type,
    ...
    for info in mmcv.track_iter_progress(nusc_infos):
        for cam in camera_types:
            ...
            (height, width, _) = mmcv.imread(
                os.path.join(args.root_path, cam_info['data_path'])).shape
            coco_2d_dict['images'].append(
                dict(
                    file_name=cam_info['data_path'][-1],
...
def generate_record(ann_rec: dict, x1: float, y1: float, x2: float, y2: float,
    ...
    if repro_rec['category_name'] not in Det3dSourceNuScenes.NameMapping:
        return None
    cat_name = Det3dSourceNuScenes.NameMapping[repro_rec['category_name']]
...
def nuscenes_data_prep(root_path,
    ...
    if version == 'v1.0-test':
        info_test_path = osp.join(out_dir,
                                  f'{info_prefix}_infos_temporal_test.pkl')
        export_2d_annotation(root_path, info_test_path, version=version)
    else:
        info_train_path = osp.join(out_dir,
                                   f'{info_prefix}_infos_temporal_train.pkl')
        info_val_path = osp.join(out_dir,
                                 f'{info_prefix}_infos_temporal_val.pkl')
...
def main(args):
    ...
    if args.version == 'v1.0-mini':
        ...
        nuscenes_data_prep(
            ...
            can_bus_root_path=args.canbus_root_path,
