...
class COCOeval:
    ...
    def __init__(self, cocoGt=None, cocoDt=None, iouType='segm', sigmas=None):
        ...
        Args:
          ...
          iouType:  type of iou to be computed, bbox for detection task,
        ...
        '''
        if not iouType:
            logging.info('iouType not specified. use default iouType segm')
            ...
            if len(p.keepDtCatIds) > 0:
                dts = self.cocoDt.loadAnns(
                    self.cocoDt.getAnnIds(
                        imgIds=p.imgIds, catIds=p.keepDtCatIds))
            else:
                dts = self.cocoDt.loadAnns(
                    self.cocoDt.getAnnIds(imgIds=p.imgIds))

            if len(p.keepGtCatIds) > 0:
                gts = self.cocoGt.loadAnns(
                    self.cocoGt.getAnnIds(
                        imgIds=p.imgIds, catIds=p.keepGtCatIds))
            else:
                gts = self.cocoGt.loadAnns(
    ...
    def evaluate(self):
        ...
        toc = time.time()
        logging.info('Per image evaluation DONE (t={:0.2f}s).'.format(toc -
                                                                      tic))
    ...
    def evaluateImg(self, imgId, catId, aRng, maxDet):
        ...
        Args:
          ...
          aRng:  area range, tuple
          maxDet:  maximum detection number
        Return:
    ...
    def accumulate(self, p=None):
        ...
        toc = time.time()
        logging.info('Accumulating DONE (t={:0.2f}s).'.format(toc - tic))
    ...
    def makeplot(self, recThrs, precisions, name, save_dir=None):
        ...
        for aidx in range(precisions.shape[2]):
            ...
            ds = np.concatenate((np.expand_dims(ps[0],
                                                axis=0), np.diff(ps, axis=0)))
            colors = [[1.0, 1.0, 1.0], [0.5, 0.5, 0.5], [.31, .51, .74],
                      [.75, .31, .30], [.36, .90, .38], [.50, .39, .64],
                      [1.0, .6, 0.0]]
            # format legend str
            ...
            if save_dir is not None:
                ...
                logging.info('save figure to %s' % save_file)
